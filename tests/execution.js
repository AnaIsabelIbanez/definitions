...
Test('/execution', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
        ...
        /**
         * summary: update or create execution
         * description: 
         * parameters: execution
         * produces: 
         * responses: 200, default
         */
        t.test('test updateTask post operation', function (t) {
            Mockgen().requests({
                path: '/execution',
                operation: 'post'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/execution']['post']['responses']['200']['schema']);
         ...
         * responses: 200, default
         */
        t.test('test updateTask put operation', function (t) {
            Mockgen().requests({
                path: '/execution',
                operation: 'put'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/execution']['put']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
